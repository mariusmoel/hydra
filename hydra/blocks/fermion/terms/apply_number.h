...
#include <hydra/blocks/fermion/terms/apply_term_diag.h>
...
void apply_number(Bond const &bond, IndexingIn &&indexing_in,
              IndexingOut &&indexing_out, Fill &&fill) {
  assert(bond.coupling_defined());
  assert(bond.type_defined() && (bond.type() == "NUMBER"));
  assert(bond.size() == 1);
  ...
  if (indexing_in == indexing_out) {
    ...
    fermion::apply_term_diag<bit_t, coeff_t>(indexing_in, term_coeff, fill);
  } else {
